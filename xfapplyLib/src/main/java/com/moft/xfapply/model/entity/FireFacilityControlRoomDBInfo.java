...
/**
 * 消防控制室-消防控制室(fire_facility_control_room)
 * 
 * @author wangx
 * @version 1.0.0 2018-05-22
 */
@Table(name = "fire_facility_control_room")
public class FireFacilityControlRoomDBInfo implements IFireFacilityInfo {
    /** 版本号 */
    @Transient
    private static final long serialVersionUID = -5164441099401924676L;
    ...
    /** 自动报警
 */
    @Property(column = "auto_alarm_usable")
    private Boolean autoAlarmUsable;
    /** 联动控制
 */
    @Property(column = "linkage_control_usable")
    private Boolean linkageControlUsable;

    @Transient
    private List<PropertyDes> pdListDetail = new ArrayList<PropertyDes>();

    public FireFacilityControlRoomDBInfo() {
        count = 1;
        autoAlarmUsable = true;
        linkageControlUsable = true;
    }

    public List<PropertyDes> getPdListDetail() {
        pdListDetail.clear();

        pdListDetail.add(new PropertyDes("消防控制室", uuid, PropertyDes.TYPE_NONE, FireFacilityControlRoomDBInfo.class));
        pdListDetail.add(new PropertyDes("名称*", "setName", String.class, name, PropertyDes.TYPE_EDIT, true));
        pdListDetail.add(new PropertyDes("自动报警", "setAutoAlarmUsable", Boolean.class, autoAlarmUsable, LvApplication.getInstance().getUsableDicList()));
        pdListDetail.add(new PropertyDes("联动控制", "setLinkageControlUsable", Boolean.class, linkageControlUsable, LvApplication.getInstance().getUsableDicList()));
    ...
    public Boolean getLinkageControlUsable() {
        return this.linkageControlUsable;
    }
