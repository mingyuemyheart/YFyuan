...
/**
 * 消防水系统-水池(fire_facility_water_pool)
 * 
 * @author wangx
 * @version 1.0.0 2018-05-22
 */
@Table(name = "fire_facility_water_pool")
public class FireFacilityWaterPoolDBInfo implements IFireFacilityInfo {
    /** 版本号 */
    @Transient
    private static final long serialVersionUID = -2105101845420496532L;
 ...
 */
    ...
    public List<PropertyDes> getPdListDetail() {
        ...
        pdListDetail.add(new PropertyDes("消防水池", uuid, PropertyDes.TYPE_NONE, FireFacilityWaterPoolDBInfo.class));
        pdListDetail.add(new PropertyDes("名称*", "setName", String.class, name, PropertyDes.TYPE_EDIT, true));
        pdListDetail.add(new PropertyDes("容量(m³)", "setCapacity", Double.class, capacity, PropertyDes.TYPE_EDIT));
        pdListDetail.add(new PropertyDes("补给速度(L/s)", "setSupplySpeed", Double.class, supplySpeed, PropertyDes.TYPE_EDIT));
        pdListDetail.add(new PropertyDes("补给方式", "setSupplyWay", String.class, supplyWay, LvApplication.getInstance().getCompDicMap().get("POOL_SUPPLY_WAY")));
        pdListDetail.add(new PropertyDes("是否有取水口(井)", "setIntakeInstalled", Boolean.class, intakeInstalled, LvApplication.getInstance().getWhetherDicList()));
        pdListDetail.add(new PropertyDes("取水口(井)位置", "setIntakeLocation", String.class, intakeLocation, PropertyDes.TYPE_EDIT));
