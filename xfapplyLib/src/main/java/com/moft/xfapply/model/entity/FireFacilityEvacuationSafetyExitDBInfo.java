...
/**
 * 安全疏散设施-安全出口(fire_facility_evacuation_safety)
 * 
 * @author wangx
 * @version 1.0.0 2018-05-22
 */
@Table(name = "fire_facility_evacuation_safety_exit")
public class FireFacilityEvacuationSafetyExitDBInfo implements IFireFacilityInfo {
    /** 版本号 */
    @Transient
    private static final long serialVersionUID = -4803255509491004940L;
 ...
 */
    ...
    public List<PropertyDes> getPdListDetail() {
        ...
        pdListDetail.add(new PropertyDes("安全出口(层)", uuid, PropertyDes.TYPE_NONE, FireFacilityEvacuationSafetyExitDBInfo.class));
    ...
    public void setFirstFloorByPartition(IPartitionInfo info) {
        if(info instanceof PartitionFloorDBInfo) {
            PartitionFloorDBInfo partitionFloor = (PartitionFloorDBInfo) info;
            if(AppDefs.KeyUnitFloorType.FLOOR_TYPE_FIRST_FLOOR.toString().equals(partitionFloor.getFloorType())) {
                firstFloor = true;
            } else {
                firstFloor = false;
            }
