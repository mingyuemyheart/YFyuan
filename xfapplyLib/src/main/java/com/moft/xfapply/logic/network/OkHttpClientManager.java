...
public class OkHttpClientManager {
    ...
    private OkHttpClientManager() {
        //cookie enabled
        final String deviceId = TelManagerUtil.getInstance().getDeviceId();
        mOkHttpClient = new OkHttpClient.Builder()
                ...
                .addInterceptor(new Interceptor() {
                    ...
                    public Response intercept(Chain chain) throws IOException {
                        ...
                        if (token != null && alreadyHasAuthorizationHeader(originalRequest, token)) {
                            return chain.proceed(originalRequest);
                        }
                        Request authorised = originalRequest.newBuilder()
                                .header("Authorization", token)
                                .header("user-agent", AppDefs.SubmitWay.MOBILE.toString())
    ...
    public final String convertToURL(String methodName) {

        return "http://" + PrefSetting.getInstance().getServerIP() + ":" +
                PrefSetting.getInstance().getServerPort() + Constant.SERVICE_NAME + methodName;
    }

    public final String convertToURL(String port, String methodName) {

        return "http://" + PrefSetting.getInstance().getServerIP() + ":" + port + methodName;
    }
     ...
     * @return
     */
    public void postAsyn(String url,final ResultCallback callback, String jsonStr) {
     ...
     * @throws IOException
     */
    public void postAsyn(String url, ResultCallback callback, File[] files, Map<String, String> params) {
        Param[] paramsArr = map2Params(params);
        String[] fileKeys = file2FileKeys(files);
     ...
     * @throws IOException
     */
    public void postAsyn(String url, ResultCallback callback, String[] fileKeys, File[] files, Map<String, String> params) {
        Param[] paramsArr = map2Params(params);
        Request request = buildMultipartFormRequest(url, files, fileKeys, paramsArr);
        deliveryResult(callback, request);
    }
     ...
     * @param params
     */
    public void putAsyn(String url, final ResultCallback callback, Param... params) {
     ...
     * @param params
     */
    public void putAsyn(String url, final ResultCallback callback, Map<String, String> params) {
        Param[] paramsArr = map2Params(params);
     ...
     * @return
     */
    public void putAsyn(String url,final ResultCallback callback, String jsonStr) {
        Request request = buildPutRequest(url, jsonStr);
        deliveryResult(callback, request);
    }
...
//    }
    ...
    private String[] file2FileKeys(File[] files) {
        String[] fileKeys = null;
        if (files != null && files.length > 0) {
            fileKeys = new String[files.length];
            for(int index = 0; index < fileKeys.length; ++index) {
                fileKeys[index] = String.valueOf(index);
            }
        }
        return fileKeys;
    }
    ...
    private Request buildPutRequest(String url, String jsonStr) {
        RequestBody requestBody = RequestBody.create(JSON, jsonStr);
        return new Request.Builder()
                .url(url)
                .put(requestBody)
    ...
    private boolean alreadyHasAuthorizationHeader(Request originalRequest, String token) {
        boolean ret = false;
        Headers headers = originalRequest.headers();
        if(headers.size() > 0 ) {
            if (headers.name(0).equals("Authorization") && headers.value(0).equals(token)) {
                ret = true;
            }
        }
        return ret;
