...
public class LogUtils {
    ...
    public static void init(boolean logSwitch, boolean log2FileSwitch, char logFilter, String tag) {
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            dir = LvApplication.getInstance().getExternalCacheDir().getPath() + File.separator;
        } else {
            dir = LvApplication.getInstance().getCacheDir().getPath() + File.separator;
        }
    ...
    public static Builder getBuilder() {
        if(StringUtil.isEmpty(dir)) {
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
                dir = LvApplication.getInstance().getExternalCacheDir().getPath() + File.separator + "log" + File.separator;
            } else {
                dir = LvApplication.getInstance().getCacheDir().getPath() + File.separator + "log" + File.separator;
            }
        }
        return new Builder();
    }
    ...
    private synchronized static void log2File(final char type, final String tag, final String content) {
        ...
        final String fullPath = dir + appId + date + ".log";
        if (!FileUtil.isFileExists(fullPath)) {
