...
package com.moft.xfapply.utils.cluster.clustering.view;
...
import com.moft.xfapply.utils.cluster.MarkerManager;
...
import com.moft.xfapply.utils.cluster.projection.Point;
import com.moft.xfapply.utils.cluster.projection.SphericalMercatorProjection;
import com.moft.xfapply.utils.cluster.ui.IconGenerator;
...
import static com.moft.xfapply.utils.cluster.clustering.algo.NonHierarchicalDistanceBasedAlgorithm.MAX_DISTANCE_AT_ZOOM;
...
//            }
            ...
            final Set<MarkerWithPosition> newMarkers = Collections.newSetFromMap(
                    new ConcurrentHashMap<MarkerWithPosition, Boolean>());
            LatLng left = UtilMap.getInstance().getLeftCorner(mMap);
            LatLng right = UtilMap.getInstance().getRightCorner(mMap);
            for (Cluster<T> c : clusters) {
                if (UtilMap.getInstance().compareLat(left, right,c.getPosition())) {
    ...
    protected void onBeforeClusterRendered(Cluster<T> cluster, MarkerOptions markerOptions) {
        ...
        if (descriptor == null) {
            descriptor = BitmapDescriptorFactory.fromBitmap(mIconGenerator.makeIcon(bucket));
