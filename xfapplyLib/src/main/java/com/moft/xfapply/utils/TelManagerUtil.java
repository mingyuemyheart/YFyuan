...
public class TelManagerUtil {

	public static TelManagerUtil mInstance = null;
	private TelephonyManager tm;

	private TelManagerUtil() {
		tm = (TelephonyManager) LvApplication.getInstance()
				.getSystemService(
				        LvApplication.getInstance().TELEPHONY_SERVICE);
	};

	public static TelManagerUtil getInstance() {
		if (mInstance == null) {
			mInstance = new TelManagerUtil();
		}
		return mInstance;
	}
	...
	public String getDeviceId() {
		...
		if (StringUtil.isEmpty(deviceId)) {
			deviceId = Settings.Secure.getString(LvApplication.getInstance().getContentResolver(), Settings.Secure.ANDROID_ID);
			if (deviceId == null) {
				deviceId = "";
			}
		}
	...
	public String getMacAddress() {
		WifiManager wifi = (WifiManager) LvApplication.getInstance()
