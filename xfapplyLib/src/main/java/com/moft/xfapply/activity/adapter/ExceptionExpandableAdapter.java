 ...
 */
public class ExceptionExpandableAdapter extends BaseExpandableListAdapter {
    private Context context;
    private List<ExceptionCategory> groupList;
    private OnGroupClickListener listener;

    public ExceptionExpandableAdapter(Context context, List<ExceptionCategory> list) {
    ...
    public View getGroupView(final int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        if (null == convertView) {
            ...
            viewHolder.tv_update_date.setVisibility(View.INVISIBLE);
        } else {
            viewHolder = (GroupViewHolder) convertView.getTag(R.id.item_view_holder);
        }
        ExceptionCategory exception = groupList.get(groupPosition);

        convertView.setTag(exception);
        if(isExpanded) {
        ...
        viewHolder.tv_name.setText(exception.getName());
        viewHolder.tv_count.setText(String.format("异常数（%s）", StringUtil.get(exception.getExceptions().size())));
