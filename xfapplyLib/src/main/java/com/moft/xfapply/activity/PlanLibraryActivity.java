...
import com.moft.xfapply.activity.adapter.PlanLibraryAdapter;
...
public class PlanLibraryActivity extends BaseActivity implements
    ...
    private PlanLibraryAdapter adapter;
    private List<PlanLibraryDTO> list = new ArrayList<PlanLibraryDTO>();
    String catalogUuid;
...
//        pageItem = PrefSetting.getInstance().getPageNum();   // 不分页
    ...
    private void initData() {
        catalogUuid = getIntent().getStringExtra("catalogUuid");
        if (StringUtil.isEmpty(catalogUuid)) {
            Toast.makeText(PlanLibraryActivity.this, "预案分类错误，请检查数据", Toast.LENGTH_SHORT).show();
            finish();
        }
        loadData();
    }
    ...
    protected void initView() {
        ...
        re_search_key.setVisibility(View.GONE);
        ...
        adapter = new PlanLibraryAdapter(this, list, new PlanLibraryAdapter.OnItemSelectedListener() {
            @Override
            public void onItemClick(PlanLibraryDTO info) {
                ...
                if (AppDefs.MediaFormat.DOC.toString().equals(info.getMediaFormat()) ||
                    ...
                    Toast.makeText(PlanLibraryActivity.this, "文件格式未知，无法打开", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onItemLongClick(PlanLibraryDTO info) {
    ...
    private void doTakeFileAction(final PlanLibraryDTO af) {
                ...
                new OkHttpClientManager.ResultCallback<String>() {
                    ...
                    public void onError(Request request, Exception e) {
                        ...
                        if (e instanceof FileNotFoundException) {
                            Toast.makeText(PlanLibraryActivity.this, "服务器文件不存在!", Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(PlanLibraryActivity.this, "网络异常!", Toast.LENGTH_SHORT).show();
                        }
                    }
                    ...
                    public void onResponse(String filePath) {
                        if (!StringUtil.isEmpty(filePath)) {
                            try {
                                ...
                                Toast.makeText(PlanLibraryActivity.this, "打开文档失败，请确认是否安装office插件", Toast.LENGTH_SHORT).show();
                            }
                        } else {
                            Toast.makeText(PlanLibraryActivity.this, "下载失败", Toast.LENGTH_SHORT).show();
                        }
                        dialog.dismiss();
                    }
                });
    ...
    private void reloadData() {
        ...
        start = 0;
        if (list == null) {
            list = new ArrayList<PlanLibraryDTO>();
        }
        list.clear();
    ...
    private void loadData() {
        ...
        http.postAsyn(http.convertToURL(String.format(Constant.METHOD_GET_PLAN_LIBRARY_LIST, catalogUuid)),
                new OkHttpClientManager.ResultCallback<ArrayRestResult>() {
            @Override
            public void onError(Request request, Exception e) {
                Toast.makeText(PlanLibraryActivity.this, "网络请求失败，请检查网络连接！", Toast.LENGTH_SHORT).show();
            }
            ...
            public void onResponse(ArrayRestResult result) {
                if (result != null && result.isSuccess()) {
                    ...
                    Toast.makeText(PlanLibraryActivity.this, "数据请求失败！" , Toast.LENGTH_SHORT).show();
                }
            }
        }, params);
    }
