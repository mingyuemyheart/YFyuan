...
import com.moft.xfapply.activity.logic.TreeLogic;
...
public class TreeAdapter extends BaseAdapter {
    ...
    private TreeLogic.OnSelectedListener listener = null;
    ...
    public void setListener(TreeLogic.OnSelectedListener listener) {
    ...
    private void closeExpand(TreePoint treePoint) {
        treePoint.setExpand(false);
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            holder.re_text = convertView.findViewById(R.id.re_text);
            holder.re_icon = convertView.findViewById(R.id.re_icon);
        ...
        ViewGroup.MarginLayoutParams margin = new ViewGroup.MarginLayoutParams(holder.re_icon.getLayoutParams());
        margin.leftMargin = 25 * level + 10;
        RelativeLayout.LayoutParams layoutParams = new RelativeLayout.LayoutParams(margin);
        holder.re_icon.setLayoutParams(layoutParams);

        if ("0".equals(tempPoint.getISLEAF())) {  //如果为父节点
            if (!tempPoint.isExpand()) {    //不展开显示加号
                holder.re_icon.setVisibility(View.VISIBLE);
                holder.icon.setImageResource(R.drawable.tree_out);
            } else {                        //展开显示减号
                holder.re_icon.setVisibility(View.VISIBLE);
                holder.icon.setImageResource(R.drawable.tree_in);
            }
        } else {   //如果叶子节点，不占位显示
            holder.re_icon.setVisibility(View.INVISIBLE);
        }
        ...
        holder.re_text.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                TreePoint treePoint = (TreePoint) getItem(position);
                if ("1".equals(treePoint.getISLEAF())) {
                    if (listener != null) {
                        listener.onSelected(treePoint.getID());
                    }
                } else {
                    if (treePoint.isExpand()) {
                        closeExpand(treePoint);
                    } else {
                        treePoint.setExpand(true);
                    }
                    ...
                    if (listener != null) {
                        listener.onSelectedParentNode(treePoint.getID());
                    }
                }
            }
        });
